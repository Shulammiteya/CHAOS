	...
	void Update () {
        if (a == 0 && m == 0)
        {
            for (int i = 0; i < num/2; i++)
            {
                ...
                monster[i].transform.position = new Vector3(Random.Range(-85.0f, 6.0f), -25.9f, Random.Range(-33.0f, 43.0f));
            }
            for(int i = num / 2; i < num; i++)
            {
                monster[i] = Instantiate(c);
                monster[i].transform.tag = "Enemy";
                monster[i].transform.position = new Vector3(Random.Range(-54.0f, 1.0f), -76.1954f, Random.Range(-17.0f, 79.0f));
            }
            m = 1;
            Debug.Log(" Create");
        }
