...
public class CreatnDestroy_g1 : MonoBehaviour {
        ...
        if (Input.GetMouseButtonDown(0))//0=brick 1=dirt 2=glass 3=gold_ore 4=grass 5=lamp 6=log 7=stone 8=none
        {
            ...
            if (Physics.Raycast(ray, out raycastHit))
            {
                ...
                if (Mathf.Abs(raycastHit.point.x - transform.localPosition.x)<2&& Mathf.Abs(raycastHit.point.y - transform.localPosition.y) < 2 && Mathf.Abs(raycastHit.point.z - transform.localPosition.z) < 2 )
                {
                    ...
                    //Hsin
                    if (raycastHit.collider.gameObject.transform.tag == "TreeHouse" && rigidBody.position.x < -24 && rigidBody.position.x > -27 && rigidBody.position.z > -2)
                    ...
                    if (create == 1)
                    {
                        ...
                        if (c.transform.position.y < -26)
                            c.transform.localScale = new Vector3(3 * c.transform.localScale.x, c.transform.localScale.y, 3 * c.transform.localScale.z);
